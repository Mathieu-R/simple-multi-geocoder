...
export async function HereAutocomplete(
	...
	const searchParams = getSearchParamsObject(options, providers.here.options.autocomplete)
  const response = await ky<HereAutocompleteResponse>(url, { searchParams: createURLSearchParams(searchParams) }).json();
